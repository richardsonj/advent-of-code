...
const createInstruction = (raw) => {
  ...
  if (matches) {
    return (value) => {
      ...
      for (let x = 0; x < value.length; x++) {
        const amount = 1 + x + (x >= 4 ? 1 : 0);
        const target = (x + amount) % value.length;
        mappings[target] = -amount;
      }
      const index = value.indexOf(matches[1]);
      rotate(value, mappings[index]);
    };
  }
  ...
  if (matches) {
    return (value) => {
      move(value, parseInt(matches[2]), parseInt(matches[1]));
    };
  }
