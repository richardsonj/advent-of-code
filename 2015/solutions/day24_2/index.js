...
const calculateSolution = (presents) => {
  let target = presents.reduce((acc, curr) => acc + curr) / 4;
...
const findOptimalGroupOfTarget = (
  ...
  for (let x = lastIndex - 1; x >= 0; x--) {
    ...
    if (sum(...newGroup) === target) {
      if (canTotalTarget(newPresents, target, [], 2)) {
...
const canTotalTarget = (presents, target, group = [], groupsNeeded = 0) => {
  for (let x = presents.length - 1; x >= 0; x--) {
    ...
    if (canTotalTarget(newPresents, target, newGroup, groupsNeeded)) {
      if (
        groupsNeeded === 1 ||
        canTotalTarget(newPresents, target, [], groupsNeeded - 1)
      ) {
        return true;
      }
    }
