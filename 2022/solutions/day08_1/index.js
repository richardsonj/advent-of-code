  ...
  for (let x = 0; x < input.length; x++) {
    for (let y = 0; y < input[0].length; y++) {
      for (let x2 = 0; x2 <= x; x2++) {
        if (x === x2) { input[x][y].visible = true;}
        else if (input[x2][y].height >= input[x][y].height) { break; }
      }
      ...
      for (let y2 = 0; y2 <= y; y2++) {
        if (y === y2) { input[x][y].visible = true;}
        else if (input[x][y2].height >= input[x][y].height) { break; }
      }
