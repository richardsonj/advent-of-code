...
const calculateSolution = (input) => {
  const state = { pressureRelieved: 0, meRoom: "AA", rooms: {}, minutesRemaining: 26, meIdleTime: 0, elephantRoom: "AA", elephantIdleTime: 0 };
...
const makeMove = (roomData, state) => {
  ...
  if (state.meIdleTime === 0) {
    for (let room of rooms.filter((room) => room.meScore > 0)) {
    ...
    for (let room of rooms.filter((room) => room.elephantScore > 0)) {
      const nextState = {
        ...state,
        rooms: {
          ...Object.keys(state.rooms)
